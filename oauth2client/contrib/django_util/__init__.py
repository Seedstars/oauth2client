...
def _load_client_secrets(filename):
    ...
    if client_type != clientsecrets.TYPE_WEB:
        raise ValueError(
            'The flow specified in {} is not supported, only the WEB flow ' 'type  is supported.'.format(client_type)
        )
...
class OAuth2Settings(object):
    ...
    def __init__(self, settings_instance):
        ...
        self.request_prefix = getattr(
            settings_instance, 'GOOGLE_OAUTH2_REQUEST_ATTRIBUTE', GOOGLE_OAUTH2_REQUEST_ATTRIBUTE
        )
        ...
        if middleware_settings is None:
            raise exceptions.ImproperlyConfigured(
                'Django settings has neither MIDDLEWARE nor MIDDLEWARE_CLASSES' 'configured'
            )
...
def get_storage(request):
    ...
    if storage_model:
        ...
        # use session
        return dictionary_storage.DictionaryStorage(request.session, key=_CREDENTIALS_KEY)
